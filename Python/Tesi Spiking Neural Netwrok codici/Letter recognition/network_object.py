...
class Network_slayer: 
     ...
     def backward(self,obj):
        ...
        for l in range(self.num_layer-2,-1,-1):  
              ...
              norm=np.max(np.abs(self.e[l]))
              if norm!=0: self.e[l]=self.e[l]/norm
...
class Objective_spike_letters:
   winner_freq=0.7
   Num_step=0

   def Calculate(self,start,stop,label):
      S_obj=np.zeros((26,self.Num_step))
     
      neur_seq=[]
      for i in label:
         neur_seq.append(ord(i)-97)
      neur_seq=np.array(neur_seq)

      Wl=int((stop-start)/len(label))
      
      i=0
      for n in neur_seq:
       S_obj[n,(start+i*Wl):(start+(i+1)*Wl)]=self.winner_freq
       i+=1
