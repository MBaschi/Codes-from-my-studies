...
class LIF_neuron: #neurone leaky integrate and fire
    ...
    def update(self,I_in): #simula il neurone dal tempo t al t+1 (questa funzione dovrà essere inserita in un for)
        ...
        if self.resting_state==False:
          ...
          if self.NOISE: self.U+=np.random.normal(loc=self.noise_mu, scale=self.noise_var)
...
class Network_slayer: 
     ...
     def backward(self,Num_step,obj,winner_neur):
        ...
        #devanisher=0 #there is problem with vanishing gradient so i add that term
        for l in range(self.num_layer-2,-1,-1):  
              grad_w=np.matmul(self.sigma[l+1],self.P_record[l].transpose())
              #grad_w=(10^(devanisher))*np.matmul(self.sigma[l+1],self.P_record[l].transpose())
     ...
     def Calculate_winner_neur(self):
        winner_neur=-1
        max=0
        for i in range(self.output_dim):
            count=np.count_nonzero(self.layer[-1].neur[i].S_record)
            if count>max:
               max=count
               winner_neur=i
        if winner_neur==-1: return self.output_dim+1
...
class Objective_spike_bin: 
    ...
    def Calculate(self,label,start=0,stop=Num_step): 
        ...
        if not self.frequency_coding:
         ...
         #spike eratte randomicamente
         if self.type==5:
            for l in range(self.Num_label):
              if l==label:
                  S_obj[l,:]=bernoulli.rvs(self.winner_freq, size=self.Num_step)
              if l!=label:
                  S_obj[l,:]=bernoulli.rvs(self.loser_freq, size=self.Num_step)
            
        return S_obj     
     
    def ones_zero(self,duration,periodicity):# ritorna un vettore di durata duration, sotto tutti 0 tranne ogni periodicity che ho un 1
        v=np.zeros(duration)
...
class Synapse_decolle(synapse): #la sinapsi in Decolle è identica a una sinapsi normale ma può essere aggiornata utilizzando l'errore del layer sucessivo e alcune informazioni salvate nel layer precedente e quello successivo
   ...
   def __init__(self,dim_pre, dim_post,mean,var,save): 
      ...
      if save:
           ...
           for i in range(self.dim_post):
              self.W_record.append([])
              self.W_record_epoch.append([])
              for j in range(self.dim_pre):
                 self.W_record[i].append([])
                 self.W_record_epoch[i].append([])
   ...
   def update_val(self,E,layer_prec,layer_post):
      ...
      for i in range(self.dim_post):
         for j in range(self.dim_pre):
            #if self.W[i,j]<0: self.W[i,j]=0.1 #se voglio che rimangano positivi
            if self.save: self.W_record[i][j].append(self.W[i,j])
   ...
   def reset(self):
      if self.save:
           for i in range(self.dim_post):
             for j in range(self.dim_pre):
              self.W_record_epoch[i][j].append(copy(self.W_record[i][j][-1]))
           self.W_record=[]
           for i in range(self.dim_post):
              self.W_record.append([])
              for j in range(self.dim_pre):
   ...
   def show(self,layer_number=0,interactive=False,start_blank=False):
      ...
      if self.save:
        ...
        if not interactive:
           plt.figure(f"Synaptic weight of layer {layer_number} trough time")
           plt.suptitle(f"Synaptic weight of layer {layer_number} trough time")
           plt.xlabel("Time step")
           plt.ylabel("Synaptic weight")
           ...
           plt.figure(f"Synaptic weight of layer {layer_number} trough epoch")
           plt.suptitle(f"Synaptic weight of layer {layer_number} trough epoch")
           plt.xlabel("Epoch")
           plt.ylabel("Synaptic weight")
  
           for i in range(self.dim_post):
              for j in range(self.dim_pre):
                plt.plot(self.W_record_epoch[i][j])
...
class Network_decolle: 
     ...
     def attach_layers(self):
        ...
        for l in range(self.num_layer-1): #attacco sinapsi
           self.synapse.append(Synapse_decolle(self.dim[l],self.dim[l+1],mean=self.meanW,var=self.varW,save=self.saveW)) #LIF_neuron.treshold/LIF_neuron.alfa*(1/(self.dim[l]+self.dim[l+1]))
     ...
     def attach_int_layers(self):
        for l in range(self.num_layer): 
             if l==0 or l==self.num_layer-1: #se il layer è quello di ingresso o finale appendiamo una lista vuota
               ...
               self.int_syn.append(Synapse_decolle(self.dim[l],self.output_dim,mean=self.meanW_int[l-1],var=self.varW,save=0))
        ...
        for n in range(num_step):
          ...
          for l in range(self.num_layer):
            ...
            if l!=0 and l!=self.num_layer-1: #layer intermedio       
              ...
              E[l]=np.matmul(e*self.int_layer[l].surr_grad,self.int_syn[l].W)/self.E_max[l-1]
     ...
     def show(self,fig=0,S_obj='a'):
        ...
        if self.saveW:
           for l in  range(self.num_layer-1):
              self.synapse[l].show(layer_number=l,interactive=False,start_blank=False)
...
class Network_slayer_FA: 
     ...
     def Create_Backward_matrix(self):
        for l in range(self.num_layer-1): #attacco sinapsi
           ...
           if self.type_FA=="uSF": self.B.append(np.sign(self.synapse[l].W.transpose()))
           if self.type_FA=="SW": self.B.append(copy(self.synapse[l].W.transpose()))
...
class NeuronI(LIF_neuron):
    ...
    def update_neur(self,I_in):
        ...
        self.P+=(-self.P)/self.T+spike
        if self.P>self.theta: self.El=1
        else:                 self.El=0
        
        if self.saveAll:
          self.P_record.append(self.P)
          self.El_record.append(self.El)
...
class Layer_I(): #il layer decolle è fatto di neuroni decolle quindi terrà conto delle variabili aggiunte nel neurone decolle
    plot_type=0
    ...
    def __init__(self,dim,saveU=0,saveS=0,saveAll=0):
        ...
        self.neur=[]
        for i in range(dim):
            self.neur.append(NeuronI(saveU,saveS,saveAll))
        self.El=np.zeros(dim)
    
    def update_layer(self,I): 
        spike=np.zeros(self.dim)
        for i in range(self.dim):
            spike[i]=self.neur[i].update_neur(I[i])
            self.El[i]=self.neur[i].El
        return spike #ritorno il vettore dei neuroni che hanno avuto una spike
    
    def reset_layer(self):
        self.El=np.zeros(self.dim)
        for i in range(self.dim):
            self.neur[i].reset_neur()
    ...
    def Algorithm_var_plot(self,layer_number=0): 
        ...
        if self.neur[0].saveAll:
           #plot traccia neuroni precednti
           plt.figure(f"P of layer {layer_number}")
           for i in range(self.dim):
            plt.plot(self.neur[i].P_record)
            #plt.axhline(y = self.neur[i].theta, color = 'k', linestyle = '--')
           plt.suptitle(f"P of layer {layer_number}")      
           plt.xlabel("Time step")
           plt.ylabel("Neuron ")
           
           #plotta eligibilità
           plt.figure(f"El of layer {layer_number}")
           for i in range(self.dim):
            E=np.array([(i+1)*j for j in self.neur[i].El_record])
            plt.plot(E,'.')
           plt.suptitle(f"El of layer {layer_number}")      
           plt.xlabel("Time step")
           plt.ylabel("Neuron ")
...
class NetwrokI:
     ...
     def __init__(self,dim,saveU=0,saveS=0,saveW=0,saveAll=0,meanW=LIF_neuron.treshold,varW=LIF_neuron.treshold):
        #parametri network
        self.dim=[]
        for i in dim:
          self.dim.append(i)
        ...
        if self.saveW:
           ...
           for l in range(self.num_layer-1): #attacco sinapsi
              self.w_record.append([])
              for i in range(self.dim[l+1]):
                 self.w_record[l].append([])
                 for j in range(self.dim[l]):
                     self.w_record[l][i].append([])
     ...
     def attach_layers(self):
        for l in range(self.num_layer): #attacco layer
            ...
            if l==self.num_layer-1:
                saveS=1
            self.layer.append(Layer_I(self.dim[l],saveU=self.saveU,saveS=saveS,saveAll=self.saveAll))
        for l in range(self.num_layer-1): #attacco sinapsi
           self.synapse.append(Synapse_slayer(self.dim[l],self.dim[l+1],mean=self.meanW,var=(2/self.dim[l]+self.dim[l+1]),save=0))
     ...
     def run(self,num_step,I_in,obj): #I_in [neurone,tempo] obj [neurone] per freq coding e [neurone,tempo] per precice spiking time
        ...
        if self.saveW:
           for l in range(self.num_layer-1):
              for i in range(self.dim[l+1]):
                 for j in range(self.dim[l]):
                    self.w_record[l][i][j].append(self.synapse[l].W[i,j])
     ...
     def backward(self,obj,S,n):
        e=obj[:,n]-S
        if self.saveAll:self.e_record[-1][:,n]=e
        for l in range(self.num_layer-2,-1,-1):  
              grad_w=np.outer(e,self.layer[l].El)
              self.synapse[l].update_val(grad_w) 
              if self.FA: e=np.matmul( self.B[l],e)          
              else:       e=np.matmul( self.synapse[l].W.transpose(),e)
              #salvo errore
              if self.saveAll:self.e_record[l][:,n]=e
     ...
     def show(self,fig=0,S_obj='a'):
        for l in  range(self.num_layer):
          if l==self.num_layer-1:self.layer[l].show(S_obj=S_obj,layer_number=l) #plot del layer finale
          if l!=self.num_layer-1:self.layer[l].show(layer_number=l) #plot dei layer interni
          if self.saveAll: #plot variabili necessarie al algoritmo
            self.layer[l].Algorithm_var_plot(l)
            if l!=0: #plot errore
             plt.figure(f"E of layer {l}")
             for e in self.e_record[l]:
              plt.plot(e.transpose())
             plt.suptitle(f"E of layer {l}")
             plt.xlabel("Time step")  

          if self.saveW and l!=self.num_layer-1:#plot pesi sinaptici
              plt.figure(f"Synaptic weight of layer {l} trough epoch")
              plt.suptitle(f"Synaptic weight of layer {l} trough epoch")
              plt.xlabel("Time step")
              plt.ylabel("Synaptic weight")
      
              for i in range(self.dim[l+1]):
                 for j in range(self.dim[l]):
                   plt.plot(self.w_record[l][i][j])
