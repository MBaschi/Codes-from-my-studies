...
class Network_decolle: 
     ...
     def run(self,num_step,I_in,obj,signal_start=0): #I_in [neurone,tempo] obj [neurone] per freq coding e [neurone,tempo] per precice spiking time
        ...
        for l in range(self.num_layer):
           ...
           if self.saveAll: self.E_record.append(np.zeros((self.dim[l],num_step)))
        ...
        for n in range(num_step):
          ...
          for l in range(self.num_layer):
            ...
            if l!=0 and l!=self.num_layer-1: #layer intermedio       
              ...
              if self.Freq_coding:
                 if n<signal_start: e=-self.int_layer[l].P 
                 else:e=obj-self.int_layer[l].P               
              else: e=obj[:,n]-S_int   
            ...
            if l==self.num_layer-1: #layer di uscita
               if self.Freq_coding:
                  if n<signal_start:E[l]=-self.layer[l].P
                  else:E[l]=obj-self.layer[l].P                              
               else: 
                  E[l]=obj[:,n]-S
     ...
     def Calculate_winner_neur(self,start=0):
       ...
       for i in range(self.output_dim):
            if np.count_nonzero(self.layer[self.num_layer-1].neur[i].S_record[start:])>S_count:
               S_count=np.count_nonzero(self.layer[self.num_layer-1].neur[i].S_record)
               neuron_index=i
       return neuron_index
