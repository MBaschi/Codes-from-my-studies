...
class Layer_decolle(): #il layer decolle è fatto di neuroni decolle quindi terrà conto delle variabili aggiunte nel neurone decolle
    ...
    def show(self,fig=0,layer_number=0,int=0):
        if self.saveS==0 and self.saveU==0: #non plottare nulla
            print("impossibile plottare perchè non è stato richiesto il salvataggio")

        if self.saveS==1 and self.saveU==0:# plot solo spike
            plt.figure(f"Spike of layer {layer_number}")
            fig+=1
            x=np.arange(0,len(self.neur[0].S_record),1)
            for i in range(self.dim):
             S=[(i+1)*j for j in self.neur[i].S_record]
        ...
        if self.saveS==0 and self.saveU==1:# plot solo potenziali
            plt.figure(f"Membrane potential of layer {layer_number}")
            fig+=1
            x=np.arange(0,len(self.neur[0].U_record),1)
            v=np.ones(len(self.neur[0].U_record))
            for i in range(self.dim):
...
class Network_decolle_I: 
     ...
     def __init__(self,dim,saveU=0,saveS=0,saveAll=0,meanW=0,varW=0.5):
        #parametri network
        self.dim=[]
        for i in dim:
          self.dim.append(i)
        ...
        for l in range(len(self.dim)): #attacco layer
            if l==self.dim:
                saveS=1
            self.layer.append(Layer_decolle(self.dim[l],saveU=saveU,saveS=saveS,saveAll=saveAll))
        for l in range(self.num_layer-1): #attacco sinapsi
           self.synapse.append(Synapse_decolle(self.dim[l],self.dim[l+1],mean=meanW,var=varW,save=saveAll))

        #layer di output intermedi per learing locale
        self.int_layer=[]
        self.int_syn=[]
        for l in range(self.num_layer): 
             if l==0 or l==self.num_layer-1: #se il layer è quello di ingresso o finale non appendaimo nulla
                self.int_syn.append([])
                self.int_layer.append([])
             else:
               self.int_syn.append(Synapse_decolle(self.dim[l],self.output_dim,mean=meanW,var=varW,save=saveAll))
               self.int_layer.append(Layer_decolle(self.output_dim,saveU=saveU,saveS=saveS,saveAll=saveAll))
        ...
        for n in range(num_step):
          ...
          for l in range(self.num_layer):
            ...
            if l!=0 and l!=self.num_layer-1:       
              ...
              E=np.matmul((S_int-S_obj[:,n])*self.int_layer[l].surr_grad,self.int_syn[l].W)
            if l==self.num_layer-1:
              E= S-S_obj[:,n]
            #aggiorna pesi sinaptici
            if l!=0:
             self.synapse[l-1].update_val(E,self.layer[l-1],self.layer[l])
            #salva l'errore
            if self.saveAll:
             if l!=0: 
              self.E_record[l][:,n]=E
     ...
     def reset(self):
        for l in range(self.num_layer):
            self.layer[l].reset_layer()

     def show(self):
        for i in  range(self.num_layer):
          self.layer[i].show(2*i,i)
     ...
     def Algorithm_var_plot(self,num_fig):
        ...
        if self.saveAll:
         fig=num_fig
         for l in  range(self.num_layer):
           self.layer[l].Algorithm_var_plot(fig,l)
           fig+=2
           if l!=0 and l!=self.num_layer-1: 
            self.int_layer[l].Algorithm_var_plot(fig,l)
            fig+=2
           if l!=0:
            plt.figure(fig)
            a=self.E_record[l]
...
class Network_decolle_II: 
     ...
     def __init__(self,dim,saveU=0,saveS=0,saveAll=0,meanW=0,varW=0.5):
        ...
        for l in range(self.num_layer): 
             if l==0 or l==self.num_layer-1: #se il layer è quello di ingresso o finale appendiamo una lista vuota
               ...
               self.int_syn.append(Synapse_decolle(self.dim[l],self.output_dim,mean=meanW,var=varW,save=0))
               self.int_layer.append(Layer_decolle(self.output_dim,saveU=saveU,saveS=saveS,saveAll=saveAll))
     ...
     def run(self,num_step,I_in,S_obj): #I_in e S_obj sono di tipo [neurone,tempo]
        ...
        for n in range(num_step):
          ...
          for l in range(self.num_layer):
            if (n==0 or n==num_step-2)and (l!=self.num_layer-1):
             print(n)
             print(self.synapse[l].W)
            #simuala layer 
            S=self.layer[l].update(I)  
            if l!=self.num_layer-1:
             I=self.synapse[l].update(S)
            ...
            if l!=0 and l!=self.num_layer-1:       
              I_int=self.int_syn[l].update(S) #layer intermedio
              S_int=self.int_layer[l].update(I_int) 
              E[l]=np.matmul((S_obj[:,n]-S_int)*self.int_layer[l].surr_grad,self.int_syn[l].W)
              #if n!=0:E[l]+=-E[l]*LIF_neuron.alfa+np.matmul((S_obj[:,n]-S_int)*self.int_layer[l].surr_grad,self.int_syn[l].W)
            if l==self.num_layer-1:
               E[l]=S_obj[:,n]-S
              #if n!=0:E[l]+=-E[l]*LIF_neuron.alfa+S_obj[:,n-1]-S
            #aggiorna pesi sinaptici
     ...
     def reset(self):
        for l in range(self.num_layer):
            self.layer[l].reset_layer()
            if l!=0 and l!=self.num_layer-1:
             self.int_layer[l].reset_layer()

     def show(self,fig):
        for l in  range(self.num_layer):
          fig=self.layer[l].show(fig,l)
          if l!=0 and l!=self.num_layer-1:
             self.int_layer[l].show(fig,l,int=1)
        return fig
