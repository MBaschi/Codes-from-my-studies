...
def plot_network(Network,obj):
   ...
   interactive=False
   start_blank=True
   for l in range(Network.num_layer-1):
      Network.synapse[l].show(layer_number=l,interactive= interactive,start_blank=start_blank)

def create_frame(frames,Network):  
    for i in  range(Network.num_layer-1):
       frames[i].append(Network.synapse[i].plot_val(layer_number=i))

def make_gif_weigth(frames,num_layer):
