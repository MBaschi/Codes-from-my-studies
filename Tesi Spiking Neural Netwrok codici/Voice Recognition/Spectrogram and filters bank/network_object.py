        ...
        if self.saveS:# plot solo spike
            ...
            if self.plot_type==0:
             for i in range(self.dim):
              ...
              plt.plot(x,S,'k.',markersize=12,label=f'{i+1}:{np.count_nonzero(S==(i+1))}')
...
class Network_slayer: 
     ...
     def run(self,num_step,I_in,obj,WINNER=0): #I_in [neurone,tempo] obj [neurone] per freq coding e [neurone,tempo] per precice spiking time
        ...
        if self.training_mode:       
           self.backward(num_step,obj,WINNER=WINNER)
           #self.num_epoch+=1
     ...
     def initialize_run(self,num_step):
        ...
        for l in range(self.num_layer):
          ...
          self.S_record.append(np.zeros((self.dim[l],num_step)))
     ...
     def backward(self,Num_step,obj,WINNER):
        ...
        if self.Freq_coding:
             ...
             if self.spike_conv_mode:
                for neur in range(self.dim[-1]):
                 if neur==WINNER:self.conv[neur,:]=np.convolve(obj[neur,:]-self.S_record[-1][neur,:],self.flip_spike_response)[Num_step-1:] 
                 else:self.conv[neur,:]=np.zeros(Num_step)
                self.e[-1]=np.power(self.conv,3)#elevo alla terza perche la differenza deve passare una funzione non lineare dispari    
             else:
                self.e[-1]=obj-self.S_record[-1]
