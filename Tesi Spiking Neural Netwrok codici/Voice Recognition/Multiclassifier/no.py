...
class Layer_slayer(): #il layer decolle è fatto di neuroni decolle quindi terrà conto delle variabili aggiunte nel neurone decolle
    ...
    def flipped_activity(self,start,num_step):
       """for n in self.dim:
          for n in range(self.Num_step):
                  if n>start and n<stop:
                    S_obj[label,n]=S_obj[label,n-1]*(1+self.alfa)+self.alfa
               S_obj =self.alfa*(S_obj -np.min(S_obj ))/(np.max(S_obj )-np.min(S_obj))"""
...
class Network_slayer: 
     ...
     def run(self,num_step,I_in,obj,winner_neur): #I_in [neurone,tempo] obj [neurone] per freq coding e [neurone,tempo] per precice spiking time
     ...
     def initialize_run(self,num_step):
        ...
        if self.spike_conv_mode:
           ...
           self.spike_response=np.power((1-self.layer[-1].neur[0].alfa),np.arange(num_step))
     ...
     def backward(self,Num_step,obj,winner_neur):
        ...
        if self.Freq_coding:
             #self.e[-1][winner_neur,:]=(obj[winner_neur,:]-self.P_record[-1][winner_neur,:]) 
             self.e[-1]=(obj-self.P_record[-1])      
             if (np.max(self.e[-1][winner_neur,:])<=0.8*LIF_neuron.alfa and np.min(self.e[-1][winner_neur,:])>=-0.8*LIF_neuron.alfa) or (winner_neur==self.Calculate_winner_neur()):
                self.e[-1][winner_neur,:]=0
        else: 
             if self.spike_conv_mode:
                if winner_neur==self.Calculate_winner_neur():
                   pass
                else:
                  for neur in range(self.dim[-1]):
                    #self.conv[neur,:]=np.convolve(obj[neur,:]-self.S_out_record[neur,:],self.flip_spike_response)[Num_step-1:] #attivita flippata
                    self.conv[neur,:]=np.convolve(obj[neur,:]-self.S_out_record[neur,:],self.spike_response)[Num_step-1:]
                  ...
                  self.e[-1]=self.conv   
                  if np.max(self.e[-1][winner_neur,:])<=0.7 and np.min(self.e[-1][winner_neur,:])>=-0.7:
                   self.e[-1][winner_neur,:]=0
             else:
                if winner_neur==self.Calculate_winner_neur():
                   pass
                else:self.e[-1]=obj-self.S_out_record 
        ...
        for l in range(self.num_layer-2,-1,-1):  
              ...
              #self.e[l]=self.e[l]/np.max(np.abs(self.e[l]))
     ...
     def Calculate_winner_neur(self,start=0):
        ...
        if self.Freq_coding:
            ...
            for i in range(self.output_dim):
                a=np.nonzero(self.layer[-1].neur[i].S_record)
                if not np.any(a):
                   pass
                else:
                    last_spike_pos=np.max(a)
                    if last_spike_pos>max:
                       max=last_spike_pos
                       winner_neur=i
