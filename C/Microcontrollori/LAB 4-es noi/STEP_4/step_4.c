// KITT 2.0 variable delay+ dynamic refresh+stopwatch format+ stopwatch precision
...
void main() {
  ...
  //VARIABILI
   ...
   unsigned short int cron_condition=0;// se è uguale a zero il cronoemtro è fermo se è uguale a 1 allora il conometro va
    ...
    unsigned short int C=0;     //valore porta in cui salvo del pin RC0
    unsigned short int C_old=0; //valore porta in cui salvo lil vecchio valore di RC0
    
    unsigned short int h=0; // ore 
    unsigned short int m=0; // minuti 
    unsigned short int s=0; // secondi
    unsigned short int ms=0; // millisecondi 
    char h_char[7];
    char m_char[7];
    char s_char[7];
    char ms_char[7];
    char tot_char[7]; // unione di tutte le cose da stampare
    strcpy(tot_char,"");
 ...
 // PORTE SETTING
  ...
  TRISC=0b00011111;// 
  ANSELC=0b11111110;// accendo buffer d'ingresso per porte 0 1 2 3 4 
...
// PROGRAMMA
  while (1){
    ...
    if (counter_2>1000 && cron_condition==1 ){ // counter 2 ha superato un secondo e il cronometro è accesso
        ...
        s+=1; // non devo preoccuparmi degli overflow di cron_val perchè le specifiche richiedono che arrivi ad un massimo di 255
      // il valore del cronometro non viene stampato qui, viene stampato solo quando premo RC0
        if (s==60){ // se raggiungo 60 secondi aumenta di 1 i minuti e riazzera i secondi
            s=0;
            m+=1;
            if (m==60){ // se raggiungo i 60 minuti aumenta di 1 le ore e azzera i minuti 
                m=0;
                h+=1;
            }
        }
  ...
  //POLLING PORTA
       ...
       if (A[2]&& A_old[2]!=A[2]){ // se premo RA2 resetta il cronometro
          ...
          h=0;
          ms=0;
          Lcd_Out(1,1,"0:0:0:0"); // mostrta che l'hai riazzerato
         }
       ...
       if (A[4] &&  A_old[4]!=A[4] && delay_kitt>delay_kitt_min){ // se premo RA4 diminuisci il kitt delay
         ...
         A_old[3]=A[3];
         A_old[4]=A[4];
 // POLLING RC0  E STAMPA ORARIO IN FORMATO h:m:s:ms
   C=PORTC.RC0;
   if (C && C_old!=C){ // se premo RC0 aggiorna il display
        // converti valori in string
          IntToStr(s,s_char);  
          IntToStr(m,m_char); 
          IntToStr(h,h_char); 
          IntToStr(ms,ms_char); 
        // attacca le stringhe di milli secondi secodni minuti e ore con i due punti in mezzo e stampa
