...
unsigned short int tmr0_overflof=0; //se =0 vuol dire che nono sono acora passati 20 ms 
void main() {
  //INIZZIALIZZAZIONE
     //VARIABILI
    ...
    //TIMER0
     T0CON=0b11001000;
     TMR0L=100;//in questo modo abbiamo overflow ogni 256*(4/fosc)*(256-TMROL)*PSA=128*126=19968us =20 ms
    //INTERRUPT
    INTCON=0b10100000;
  ...
  //PROGRAMMA
      while(1){
          ...
          if (stato_accensione==1){
               ...
               if (PORTA.RA1 && tmr0_overflof==1){
                   potenza_motore+=1;
                   tmr0_overflof=0;
                   CCPR5L=potenza_motore;
                }
              if (PORTA.RA2 && tmr0_overflof==1){
                   potenza_motore-=1;
                   tmr0_overflof=0;
                   CCPR5L=potenza_motore;
                }
