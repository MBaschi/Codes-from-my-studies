...
n_crisis=length(seg.data)/3; %numero de crisis
    ...
    for i=1:n_crisis %i=contador crisis
           ...
           for p=1:3 %contador periodo
               for b=1:5 % b contador banda              b=1-->delta
                                      ...
                                      %                  b=5-->gamma
                        for c=1:23
                            [PSD,w]=pwelch(period(p).data(:,c),hamming(20*fm),10*fm,4096,fm); %periodogramma de welch
                            Etot=sum(PSD); %energia total

                            %energia normalizada
                            NE(c)=sum(PSD((v(b)+1):(v(b+1)+1)))/Etot; %v=[1 4 8 12 30 128]*4096/fm declarado inicialmente
                            %potencia
                            P(c)=sum(PSD((v(b)+1):(v(b+1)+1)))*fm/4096;
                        end 
  ...
  Titulo_1={'','PREICTAL','','','','','ICTAL','','','','','POST','','','','',fname};
