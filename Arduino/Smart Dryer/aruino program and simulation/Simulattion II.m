...
Ni=300; %numero di molecole d'acqua nel vestito iniziali;
t_incr=100;
...
mb=@(v,T,N)N.*4.*pi.*(m./(2.*pi.*R.*T)).^(1.5).*v.^2.*exp(-(m./(2.*pi.*R.*T)).*v.^2); %distribuzione di maxwell boltzman
v=[0:10:4000]; plot(v,mb(v,T0_p,Ni)) %se vuoi plottare la distribuzione di maxweel boltzman
vf=2700; %velocità di fuga delle molecole (ipotizzata a caso guardando la distribuzione di maxwell boltzam)


dE=@(y2,y3)integral(@(v)(mb(v,y2,y3).*0.5.*m.*v.^2),vf,inf);
dN=@(y2,y3)integral(@(v)mb(v,y2,y3),vf,inf);

System=@(t,y)[c_1.*(y(2)-y(1))+c_2.*(T_amb-y(1)) %y(1)= temperatura aria, y(2)=temperatura panno, y(3)= numero di particelle nel panno  
              c_3.*(y(1)-y(2))-c_4.*dE(y(2),y(3))./t_incr
              -dN(y(2),y(3))./t_incr]; 
            
t=[1:t_incr:10^6]; %tempo di simulazione in secondi
Y0=[ T_amb
     T0_p
     Ni];
